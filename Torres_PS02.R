...
test.function<-function(x){
  ...
  for(j in 1:9){ 
    totals2[j] <- (sum(vece==j))/length(vece)-1      #Incorrect calculation
    leemvece2[j]<- totals2[j]-log10(1+ (1/j))      #Correct leemis formula
    chovece2[j]<-(totals2[j]-log10(1+ (1/j)))^2    #Correct cho formula
  }
  numse1<-1:9
